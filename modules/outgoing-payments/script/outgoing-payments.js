...
/*Load Tabs*/
	...
	//Logistics
	$('#logistics-tab').load('../templates/outgoing-payments-logistics.php'), function(){
		
	};
	//Accounting
	$('#accounting-tab').load('../templates/outgoing-payments-accounting.php'), function(){
...
//Update	
	$(document.body).on('click', '#btnUpdate', function () 
	{
        ...
        if (err == 0) 
		{
            ...
            $.ajax({
                type: 'POST',
                url: '../proc/exec/exec_update_outgoing-payments.php',
				data: 
				{
			    ...
			    success: function (data) 
				{
					...
					if(res.valid == true)
					{
						...
						$('#messageBar').text(res.msg).css({'background-color': '#00FF7F', 'color': 'black'});
							...
							window.location.replace("../templates/outgoing-payments-document.php");
							},3000)
					}
					else
					{
...
/* $('input.itemcode').on('paste', function(e){
	...
	let bplid = $('#txtBranchCode').val();
	let serviceType = $('#selTransactionType').val();
	let cardcode = $('#txtCardCode').val();

	$('#contents-tab').load('../proc/views/vw_getInvoices.php?bplid=' + bplid + '&cardcode=' + cardcode + '&serviceType=' + serviceType), function (){
