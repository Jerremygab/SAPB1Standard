$(document).ready(function () {
	...
	$(document.body).on('click', '#btnAdd', function () {
		...
		if (err == 0) {
			...
			$.ajax({
				...
				success: function (data) {
					if (data != 0) {
						alert('File Successfully Added to:' + data);
					}
					else {
						alert(' File uplad error.');
					}
				},
			});
	...
	function AddRowAttachment(fileReader) {
		...
		setTimeout(function () {
			...
			$('#rowLoader').load('../templates/' + mainFileName + '-lines-row-attachments.php?', function (result) {
				...
				$('._download-upload').on('click', function(){
					console.log('clicked')
				})
