...
const baseTableName1 = sessionStorage.baseTableName1;
const baseTable1 = sessionStorage.baseTable1;
const baseType1 = parseInt(sessionStorage.baseType1);
const copyFromModal1 = sessionStorage.copyFromModal1;
const copyFromModalTbl1 = sessionStorage.copyFromModalTbl1;
...
//Double Clicks
	...
	$(document.body).on('dblclick', '#tbl'+copyFromModalTbl1+' tbody > tr', function () 
	{
		
		var docNum = $(this).children('td.item-1').text();
		var objType = baseType1;
		
        $('#'+copyFromModal1+'Modal').modal('hide');
		...
		//Addresses
			...
			$.ajax({
				...
				success: function (data) 
				{
					...
					$('#txtStatesS').val(dataObj.StateCode);
...
//On Shown Modals
	...
	$('#'+copyFromModal1+'Modal').on('shown.bs.modal',function(){
		
		var cardCode = $('#txtCardCode').val();
		var table = baseTable1;
		
		
		if(cardCode == '')
		{
			
			$('#tbl'+copyFromModalTbl1+' tbody').html('');
		}
		else
		{	
			$.ajax({
				type: 'GET',
				url: '../proc/views/vw_copyFrom'+copyFromModalTbl1+'.php',
				data: {
					cardCode : cardCode,
					table: table
				},
				success: function (html) 
				{
					$('#'+copyFromModal1+'Result').html(html);
				}
			});
		}
	});
...
//Address
    ...
    $(document.body).on('dblclick', '#tblDocNum tbody > tr', function () 
	{
        ...
        if (lineNum) {
			...
			$('.selected-row .txtRefDocDate').val($(this).children('.docDate').text());
	...
	{
		let lineNum = hasMissingRefDocNum();
		if (lineNum) {
			portalMessage('Document number is mandatory for chosen document in line ' + lineNum + '.', 'red', 'white');
		} else {
				...
				if (dataObj.length > 0) {
					...
					setTimeout(function(){
						$('#tbldocRefTo tbody').find('tr').each(function(index) 
						{
							...
							$(this).find('.txtRefDocDate').val(dataObj[index].IssueDate);
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------		
	...
	function PreviewDoc(docNum, objType){
		...
		$.getJSON('../proc/views/vw_getheaderdata.php?docNum=' + docNum + '&objType=' + objType, function (data){
            ...
            $.each(data, function (key, val){
				...
				$('#txtRemarks').val(`Copied from ${baseTableName1} # ` + val.DocNum );	
				}
	...
	function PreviewRows(docNum, docType, objType,callback){
        $('#tblDetails tbody').load('../proc/views/vw_getdetailsdata.php?docNum=' + docNum + '&docType=' + docType + '&objType=' + objType, function (result) 
		{
			...
			if (objType == baseType1 && val[val.length - 1] == 0 && $('input.itemcode:first-child').val() != ''){
				ComputeFooterTotalBeforeDiscount();
				ComputeFooterTaxAmount();
				ComputeTotal();
			}
	...
	function loadRefDocModal() {
    	...
    	if ($.fn.DataTable.isDataTable('#tblTransactType')) {
    		$('#tblTransactType').DataTable().destroy();
		}
	...
	function hasMissingRefDocNum(){
		let result = false;
		$('#tbldocRefTo tbody').find('tr').each(function(){
			if ($(this).find('input.refDocToInput').hasClass('txtTransactType') &&
				$(this).find('input.refDocToInput').val() != '') {
				if ($(this).find('input.txtRefDocNum.refDocToInput').val() == '') {
					result = $(this).find('td.rowNo span').text();
					return false;
				}
			}
		})
